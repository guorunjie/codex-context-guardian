...
export function writeRecoveryBundle(input: RecoveryBundleInput): string {
  ...
  const handoffMemory = {
    ...buildHandoffMemory(input.thread, recentThreadContext),
    sourceFailure: {
      kind: input.signal.kind,
      confidence: input.signal.confidence,
      reason: input.signal.reason,
      sourceLogId: input.signal.sourceLogId || null
    },
    lastHealthyCheckpoint: input.healthyCheckpoint || null,
    resumeFrom: input.signal.kind === "context_overflow"
      ? "last healthy checkpoint; do not continue by compacting the saturated source thread"
      : "latest recovered thread context"
  };
  ...
  fs.writeFileSync(path.join(dir, "RECOVERY.md"), renderRecoveryReadme({
    ...
    recentThreadContext,
    handoffMemory,
    healthyCheckpoint: input.healthyCheckpoint || null,
...
- available: ${input.healthyCheckpoint ? "yes" : "no"}
- capturedAt: ${input.healthyCheckpoint?.capturedAt || "unavailable"}
- phase: ${input.healthyCheckpoint?.phase || "unavailable"}
- turnId: ${input.healthyCheckpoint?.turnId || "unavailable"}
- transcriptPath: ${input.healthyCheckpoint?.transcriptPath || "unavailable"}
...
- If the failure kind is context_overflow, recover from the last healthy checkpoint and avoid retrying the saturated source thread until the new fork has made a durable progress note.
